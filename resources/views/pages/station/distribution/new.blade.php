...
@section('content')
    <div class="container-fluid">
        <form action="{{ route('station.distribution.store') }}" method="post">
            @csrf
                                ...
                                <div class="col-lg-7">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control form-control-alternative"
                                            value="{{ Auth::user()->station->name }}" disabled>
                                        <input type="hidden" class="form-control form-control-alternative"
                                            name="station_id" id="station_id"
                                            value=" {{ Auth::user()->station->id }}" aria-describedby="helpId" placeholder=""
                                            required>
                                    </div>
                                </div>
                                <div class="col-lg-5">
                                    <div class="form-group">
                                        ...
                                        <input type="date" class="form-control form-control-alternative"
                                            name="issue_date" id="issue_date" aria-describedby="helpId" placeholder=""
                                            required>
                                    </div>
                                </div>
                            </div>
...
@section('js')
    <script>
        function addProduct() {
            ...
            console.log(pro_id);
            $.ajax({
                url: "{{ route('station.fuel-type.get') }}?id=" + pro_id,
                headers: {
                ...
                success: function(response) {
                    ...
                    if (response != []) {
                        ...
                        html = ' <tr>' +
                            ...
                            ' <td>' + response['name'] + '</td>' +
                            ' <td>' + qty + '</td>' +
                            ' <input type="hidden" class="form-control" name="fuel_type_id[]" value="' +
                            pro_id + '">' +
                            ' <input type="hidden" class="form-control" name="qty[]"' + ' value="' + qty +
                            '">' +
                            ' <input type="hidden" class="form-control" name="price[]"' + ' value="' + response[
                                'price'] + '">' +
                            ' </tr>';
